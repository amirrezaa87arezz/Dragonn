...
# --- بخش زنده نگه داشتن برای Render ---
app_web = Flask('')
@app_web.route('/')
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == 'بازگشت به منوی اصلی':
        ...
        prices = [
            [InlineKeyboardButton("20 گیگ | نامحدود - 130ت", callback_data="p_20G_نامحدود_130")],
            [InlineKeyboardButton("30 گیگ | نامحدود - 160ت", callback_data="p_30G_نامحدود_160")],
            [InlineKeyboardButton("40 گیگ | نامحدود - 190ت", callback_data="p_40G_نامحدود_190")],
            [InlineKeyboardButton("50 گیگ | نامحدود - 250ت", callback_data="p_50G_نامحدود_250")],
            [InlineKeyboardButton("100 گیگ | نامحدود - 420ت", callback_data="p_100G_نامحدود_420")]
        ]
        await update.message.reply_text("لیست پلن‌های ارزان:", reply_markup=InlineKeyboardMarkup(prices))

    elif text == 'قوی':
        prices = [
            [InlineKeyboardButton("20 گیگ | 1 ماهه - 150ت", callback_data="p_20G_1 ماهه_150")],
            [InlineKeyboardButton("50 گیگ | 1 ماهه - 280ت", callback_data="p_50G_1 ماهه_280")],
            [InlineKeyboardButton("100 گیگ | 1 ماهه - 550ت", callback_data="p_100G_1 ماهه_550")],
            [InlineKeyboardButton("200 گیگ | 3 ماهه - 1,100ت", callback_data="p_200G_3 ماهه_1100")]
        ]
        ...
        invoice = f"📇 پیش فاکتور:\n👤 نام: {text}\n🔐 سرویس: {user_data[user_id]['vol']}\n💶 قیمت: {p},000 تومان"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("p_"):
        ...
        p = user_data[user_id]['price']
        await query.message.reply_text(f"💳 کارت: <code>6277601368776066</code>\n💰 مبلغ: {p},000\nفیش را بفرستید.", 
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ارسال فیش", callback_data="get_receipt")]]), parse_mode='HTML')
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data.get(user_id, {}).get('step') == 'wait_photo':
        ...
        await context.bot.send_photo(chat_id=ADMIN_ID, photo=update.message.photo[-1].file_id, 
            caption=caption, parse_mode='HTML', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ارسال کانفیگ ✅", callback_data=f"adm_to_{user_id}")]]))
