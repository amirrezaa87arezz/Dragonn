...
ADMIN_ID = 5993860770
user_data_storage = {} # برای ذخیره موقت اطلاعات خرید کاربر
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # مرحله دریافت نام کاربری
        ...
        invoice = (f"📇 پیش فاکتور شما:\n👤 نام انتخابی: {text}\n"
                   f"🔐 سرویس: {user_data_storage[user_id]['volume']}\n"
                   f"💶 قیمت: {price},000 تومان\n💰 سفارش آماده پرداخت است")
        
        await update.message.reply_text(invoice, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ادامه و دریافت شماره کارت ✅", callback_data=f"show_card")]]))

    # پنل ادمین: ارسال کانفیگ نهایی برای کاربر
    elif user_id == ADMIN_ID and user_id in user_data_storage and user_data_storage[user_id].get('step').startswith("adm_send_"):
        target_id = int(user_data_storage[user_id]['step'].split("_")[-1])
        ...
        user_data_storage[user_id]['step'] = None
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("p_"):
        ...
        bank = (f"💳 شماره کارت:\n`6277601368776066`\n"
                ...
                f"⭕ مبلغ را دقیق واریز کرده و فیش را بفرستید.")
        await query.message.reply_text(bank, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ارسال فیش واریزی", callback_data="get_receipt")]]), parse_mode='Markdown')
        ...
        user_data_storage[ADMIN_ID] = {'step': f"adm_send_{target}"}
        await query.message.reply_text(f"لطفاً کانفیگ را برای کاربر {target} بفرستید:")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data_storage.get(user_id, {}).get('step') == 'wait_photo':
        ...
        # ارسال برای ادمین
        caption = (f"🔔 فیش جدید!\n🆔 آیدی: `{user_id}`\n👤 نام انتخابی کاربر: {info['chosen_name']}\n"
                   f"📦 پلن: {info['volume']} | {info['time']}")
        
        await context.bot.send_photo(chat_id=ADMIN_ID, photo=update.message.photo[-1].file_id, 
            caption=caption, 
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ارسال کانفیگ ✅", callback_data=f"adm_to_{user_id}")]]))
        
        await update.message.reply_text("🚀 رسید شما ارسال شد. پس از بررسی، اطلاعات سرویس برای شما ارسال می‌گردد.")
        user_data_storage[user_id]['step'] = 'done'
